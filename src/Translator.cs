...
{
    /// <summary>
    /// A class to translate WPF localization files in XAML format to another language
    /// Author: Lance Liao  www.shuyz.com
    /// </summary>
    public class Translator
    {
        ...
        public Translator(string inputFile, string outputFile, string srcLang, string dstLang)
        {
        ...
        private void ProcessFile()
        {
            ...
            {
                while (File.Exists(this._outputFile))
                {
                    this._outputFile = "1_" + this._outputFile;
                }
            ...
            Console.WriteLine("Translation done for {0} -> {1}.", this._inputFile, this._outputFile);
        }
        ...
        private string ProcessOneLine(string rawContent)
        {
            ...
            foreach(Match match in collection)
            {
                ...
                string dstStr = this.GetTranslation(srcStr);
        ...
        private string GetTranslation(string content)
        {
            ...
            // Single line: [[["自动冰柜管理系统","Automatic Freezer Management System",,,3]],,"en"]
            // Multiple line: [[["您确定要删除此用户吗？","Are you sure you want to delete this user?",,,3],["您可以禁用此用户，而不是删除。","You could disable this user instead of deleting.",,,3]],,"en"]
